...
function gridAdd(id: string, x: number, y: number): void {
...
function gridMove(id: string, ox: number, oy: number, nx: number, ny: number): void {
...
function gridRemove(id: string, x: number, y: number): void {
  const key = cellKey(x, y);
...
function encodeSnapshot(_viewerId: string, list: SnapshotEntry[]): Buffer {
...
function interestFor(player: Player): Player[] {
  const result: Player[] = [];
...
function sendFile(res: http.ServerResponse, absolutePath: string): void {
...
wss.on('connection', (ws: WebSocket) => {
  ...
  ws.on('message', async (raw: WebSocket.RawData) => {
    ...
    try {
      ...
      if (type === 'offer' && typeof message.sdp === 'string') {
        ...
        ws.send(
          JSON.stringify({ type: 'answer', sdp: pc.localDescription?.sdp ?? sdp, id }),
        );
  ...
  function handleBinaryMessage(pid: string, data: ArrayBuffer | Buffer | Uint8Array): void {
    ...
    const op = view.getUint8(0) as Opcode;
    if (op === OPC.UPDATE) {
...
setInterval(() => {
  ...
  for (const player of players.values()) {
    ...
    const snapshotEntries: SnapshotEntry[] = visible.map((p) => ({
      id: p.id,
      x: p.x,
      y: p.y,
      name: p.name,
...
function broadcastRegional(fromId: string, cx: number, cy: number, radius: number, text: string): void {
...
function sendPrivate(fromId: string, targetHash: number, text: string): void {
  const sender = players.get(fromId);
  if (!sender) {
    return;
  }
...
function forwardBeamRequest(fromId: string, targetHash: number, text: string): void {
  const sender = players.get(fromId);
  const recipient = [...players.values()].find((player) => hashId(player.id) === targetHash);
